...
export class ReceiverSupervisor {
  ...
  private buildRunArguments(config: AppConfig): string[] {
    return [
      ...
      "--color-format",
      config.receiver.colorFormat,
      "--output-fps-cap",
      String(config.receiver.outputFpsCap),
      "--low-latency-mode",
      config.receiver.lowLatencyMode ? "enabled" : "disabled",
      "--status-file",
  ...
  private async handleChildExit(
    ...
    this.applyStatus({
      ...
      droppedVideoFrames: unexpected ? this.status.droppedVideoFrames : null,
      droppedAudioFrames: unexpected ? this.status.droppedAudioFrames : null,
      videoQueueDepth: unexpected ? this.status.videoQueueDepth : null,
      audioQueueDepth: unexpected ? this.status.audioQueueDepth : null,
